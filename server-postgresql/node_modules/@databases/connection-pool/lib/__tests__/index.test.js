...
test('ConnectionPool - releaseTimeoutMilliseconds', async () => {
  ...
  const pool = __1.default({
    ...
    onReleaseTimeout: async connection => {
      timedOut.add(connection); // N.B. in a real app, you must either throw (crashing the app)
  ...
  expect(a.connection).toBe(1);
  expect(b.connection).toBe(2); // we'll just try and get 2 more without releasing the original conections

  const [c, d] = await Promise.all([pool.getConnection(), pool.getConnection()]);
  expect(c.connection).toBe(3);
  expect(d.connection).toBe(4);
  expect([...timedOut].sort()).toEqual([1, 2]); // releasing the original connections is a no-op
