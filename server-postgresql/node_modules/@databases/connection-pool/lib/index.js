...
class PoolConnectionImpl {
  ...
  release() {
    ...
    if (this._released) {
      throw errors_1.doubleReleaseError();
    }
...
class ConnectionPoolState {
  constructor(options) {
    ...
    this._onReleaseTimeout = record => {
      ...
      IPoolRecord_1.setRecordState(record, IPoolRecord_1.PoolRecordState.Disposed);
    ...
    const [drained, onDrained] = defer_1.default();
  ...
  async _openConnection() {
    const [destroyed, destroy] = defer_1.default();

    this._increaseTotalConnectionsCount();

    return RESOLVED_PROMISE.then(() => this._options.openConnection(destroy)).then(connection => {
      if (timeout_1.isTimeout(connection)) {
        throw errors_1.openTimeout();
      }

      const record = IPoolRecord_1.getPoolRecord(connection);
      const r = record;
      void destroyed.then(() => {
        if (IPoolRecord_1.isActivePoolRecord(r)) {
          r.shouldDestroy = true;
        } else if (IPoolRecord_1.isIdlePoolRecord(r)) {
  ...
  _releaseConnection(record) {
    if (!IPoolRecord_1.isActivePoolRecord(record)) {
    ...
    if (waiter) {
      ...
      this._idleConnections.push(IPoolRecord_1.setRecordState(record, IPoolRecord_1.PoolRecordState.Idle, this._onIdleTimeout, this._options.idleTimeoutMilliseconds));
    }
  }
  ...
  _closeConnection(record) {
    ...
    if (IPoolRecord_1.isDisposedPoolRecord(record)) {
  ...
  _getNextIdleRecord() {
    while (true) {
      ...
      if (record === undefined || IPoolRecord_1.isIdlePoolRecord(record)) {
        return record;
      }
    }
  }
  ...
  async getConnection() {
    ...
    const record = (_a = this._getNextIdleRecord()) !== null && _a !== void 0 ? _a : this._canOpenConnection() ? await this._openConnection() : await new Promise(resolve => {
      this._waiters.push(new Waiter_1.default(resolve, this._options.queueTimeoutMilliseconds));
    });

    if (timeout_1.isTimeout(record)) {
      throw errors_1.queueTimeoutError();
    }

    if (IPoolRecord_1.isIdlePoolRecord(record)) {
    ...
    const activeRecord = IPoolRecord_1.setRecordState(record, IPoolRecord_1.PoolRecordState.Active);
  ...
  async drain() {
    ...
    for (const connection of this._idleConnections.clear()) {
      if (IPoolRecord_1.isIdlePoolRecord(connection)) {
